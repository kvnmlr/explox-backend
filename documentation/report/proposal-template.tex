...
\def\plaintitle{ExploX - Seminar Project Report}
...
\begin{abstract}
  In this Report, we present ExploX, an web-app that allows for route generation and exploration of more new areas.With this we want to enable the user to generate a more holistic image of their living surroundings and allow them to escape their daily routine. \textbf{}
\end{abstract}
...
In the last 10 years, the services such as Google Maps that enable point to point navigation and Geocaching that enable people to explore unfamiliar areas have made great progress and bring more fun. Nowadays also a variety of services such as Strava and Komoot emerge not only navigation but also route planning for running and biking. Based on these services, we want to develop a web app which could plan routes and help do personal exploration of the unseen area with other's history data using the API of Strava, motivating athletes to cover more less visited parts of their environment.

Our aim here is to motivate athletes to explore more of less visited parts of Saarbr\"ucken. 

To accomplish this ,firstly we aim to create routes along paths around areas that the user is most unlikely to know about. The maps would be divided into two parts, the familiar area and unfamiliar area using the history data. The app would recommend athletes the routes in seldom visited area but frequent visited by others, in terms of safety.By setting the staring point ,destination and the total distance that want to be covered, our algorithm would generate several routes and the percentage of covering new area.

Secondly our work would show the black map of covered area and uncovered one. So the generated route could be downloaded and previewed before exploring.The explored areas would fade out from dark black to transparent according the the frequency of visiting.

Psychogeography is the practice of exploring the urban environment with the intention to investigate the effects on feelings and behavior. To generate a complete image of the city is necessary, as the Psychogeographers argue. This is one of the  reason we want to create the fading map.

Taking routes of another athlete could avoid the case of illegal exploration or simply routes that are not suited for cycling. One has to explore parts of the environment that are normally not frequented.As our aim with this project is not to support illegal activities we do not consider the actual exploration of such areas  which is regarded as danger.

Using the social media sources and personal location history(GPS data) we determine areas that are less frequented and might be unknown even to citizens living in the city for a long time. The design of the ExploX application, tries to combine the ideas of the psychogeography movement from the 60s with more recent developments of the exploration and Strava communities. With this we want to enable the user to generate a more holistic image of their living surroundings and allow them to escape their daily routine. Additionally we hope to complete the users spatial memory about their city.

The report is structured as follows. First we present the introduction and related work that led to the development of this idea. Afterwards we lay out the implementation details, such as the framework and the idea of combining the segmentation.Thereafter we will present how we conduct the user study for evaluation of the designs and functions we have implemented and show the related results.It would show whether the result support or against our hypotheses "the web-app motives the athletes to explore more new places".The results showe that the usability and user
experience of our system is above average overall
...
\begin{itemize}
\item Attractiveness (1.333)
\item Perspicuity (1.607)
\item Efficiency (1.429)
\item Dependability (1.357)
\item Stimulation (1.214)
\item Novelty (1.143)
\end{itemize}
