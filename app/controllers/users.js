...
exports.session = async function (req, res) {
    if (req.user.provider === 'strava') {
        ...
        if (req.user.fullyRegistered) {
            ...
            await Strava.updateUser({profile: req.user, max: 2}); // only take 2 so login does not take too long
        } else {
            // synchronize asynchronously while they are registering
            Strava.updateUser({profile: req.user, max: 2});// only take 2 so login does not take too long
        }
    }
